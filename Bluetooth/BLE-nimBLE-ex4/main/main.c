...
*   - Control a RELAY by UART Service
*   
...
void app_main(void) {
    ...
    const int RELAY_PIN = 10; // ou CONFIG_RELAY_GPIO
    relay_init(&s_relay, RELAY_PIN, RELAY_NO, 0); // NO, inicial OFF

    /* Init services */
    heart_rate_init();      // heart rate service
    battery_init();         // battery service
    led_init();             // led control service
    uart_init(&s_relay);    // uart service
