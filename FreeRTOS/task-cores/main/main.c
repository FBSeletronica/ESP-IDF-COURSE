...
void app_main(void)
{
    ...
    xTaskCreatePinnedToCore(
     ...
     ESP_LOGD(TAG1,"Iniciando a [%s]. CORE[%d]",pcTaskGetName(NULL),xPortGetCoreID());         //log message
    while(1){
        //print high water mark of each task
        ...
        vTaskDelay(pdMS_TO_TICKS(10000));   //delay of 1 second
    }
}
...
void vTaskCounter(void *pvParameters )
{
    ...
    ESP_LOGD(TAG3,"Iniciando a [%s]. CORE[%d]",pcTaskGetName(NULL),xPortGetCoreID());         //log message
    while(1){
