...
* 
*  This example shows how to use binary semaphores to synchronize an ISR and a task.
...
SemaphoreHandle_t xBinarySemaphore = NULL;    // Semaphore handle
...
static void IRAM_ATTR gpio_isr_handler(void* arg)
{
    ...
    {
        // Notify the task
        xSemaphoreGiveFromISR(xBinarySemaphore, &xHigherPriorityTaskWoken);   // Give the semaphore
    }
...
void led_task(void *pvParameter)
{
    int led_status = 0; // LED status

    while(1)
    {
        // Wait for the semaphore
        if(xSemaphoreTake(xBinarySemaphore, portMAX_DELAY) == pdTRUE) // Wait for the semaphore
        {
            gpio_set_level(LED1_PIN, led_status^=1);    // Toggle LED
            ESP_LOGI(TAG, "LED: %d", led_status);       // Log LED status
        }
    }
}