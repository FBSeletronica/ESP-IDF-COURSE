...
void vTaskCounter(void *pvParameters )
{
    ...
    while(1){
        ...
        if(i == 1010){
            vTaskSuspend(xTaskBlinkHandle);             //suspend the task
            gpio_set_level(LED_PIN,0);                  //turn off the LED
            ESP_LOGI(TAG3,"Task Blink suspended");      //log message

        }
        else if(i == 1015){
            vTaskResume(xTaskBlinkHandle);              //resume the task
            i = 1000;
            ESP_LOGI(TAG3,"Task Blink resumed");        //log message
        }
