...
static void check_message(app_message_t *message)
{
    static uint8_t state = 0;
    static uint8_t led = 0;
    for (size_t i = 0; i < message->buf_len; i++) {
        switch (message->buf[i]){
            case 'L':
                ESP_LOGI(TAG, "Received 'L'. state: %lu", state);
                state = 1;
                break;

            case 'E':
                ESP_LOGI(TAG, "Received 'E'. state: %lu", state);
                if (state == 1)
                    state = 2;
                else
                    state = 0;
            ...
            case 'D':
                ESP_LOGI(TAG, "Received 'D'. state: %lu", state);
                if (state == 2) {
                    ESP_LOGI(TAG, "Received correct message. Toggling LED");
                    led ^= 1;
            ...
            default:
                ESP_LOGI(TAG, "Received unexpected char: '%c'", message->buf[i]);
                state = 0;
        }
    }
}
